    ...
    <div className="sticky top-3 z-50 w-full py-2">
      <div className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:flex">
          {/* removed glassy blur + transparent bg */}
          <nav className="w-full max-w-screen-xl mx-auto rounded-xl border bg-background px-4 py-2 border-zinc-200">
            <div className="flex w-full items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 lg:gap-4 flex-1 justify-center">
                <Link href={`/make-money?lang=${language}`} className="px-3 py-2 hover:text-foreground transition-colors">
                  <span className={`text-sm font-medium text-black ${language === "ar" ? "font-arabic" : "font-english"}`}>
                    {content[language].referral}
                  </span>
                </Link>
                <Link href={`/projects?lang=${language}`} className="px-3 py-2 hover:text-foreground transition-colors">
                  <span className={`text-sm font-medium text-black ${language === "ar" ? "font-arabic" : "font-english"}`}>
                ...
                <button
                  ...
                  className={`px-3 py-2 hover:text-foreground transition-colors text-sm font-medium text-black ${language === "ar" ? "font-arabic" : "font-english"}`}
                >
                  {content[language].services}
                </button>
                <Link href={`/contact?lang=${language}`} className="px-3 py-2 hover:text-foreground transition-colors">
                  <span className={`text-sm font-medium text-black ${language === "ar" ? "font-arabic" : "font-english"}`}>
            ...
            <div className="mt-4 mx-4 rounded-xl border border-border/40 bg-background shadow-md">
              <div className="p-4 space-y-2">
                ...
                <div className="pt-4 border-t border-border/40">
                  <div className={`flex items-center gap-3 ${language === "ar" ? "justify-end" : "justify-start"}`}>
                    <LanguageToggle onLanguageChange={onLanguageChange} currentLanguage={language} />
                  </div>
                </div>
              </div>
